...
def startTask(user_id, task_name, interval, length, tasks):
    ...
    # start recording
    th1 = channelThread(1, 'EEG', sub_task_name + '_' + str(length), duration, time.time(), os.path.join('data', user_id))
    th2 = channelThread(2, 'PPG', sub_task_name + '_' + str(length), duration, time.time(), os.path.join('data', user_id))
...
if  __name__ == "__main__":
    # N-back configs
    tasks_ = [3,4,5]
    task_name_ = 'T' # T for training, R for recorded
    user_id_ = 'x'
    interval_ = 2.25
    length_ = 30
